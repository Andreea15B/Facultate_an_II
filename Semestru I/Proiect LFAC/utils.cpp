...
bool WordIsReserved(char *word) {
    if(strcmp(word, "spreadTheSpirit") == 0) return true;
    if(strcmp(word, "reindeer") == 0) return true;
    if(strcmp(word, "if you love Christmas you get") == 0) return true;
    if(strcmp(word, "for each gift") == 0) return true;
    if(strcmp(word, "while Christmas goes on") == 0) return true;
    if(strcmp(word, "else you're a Grinch") == 0) return true;
    if(strcmp(word, "Christmas") == 0) return true;
    if(strcmp(word, "private gifts") == 0) return true;
    if(strcmp(word, "public gifts") == 0) return true;
    return false;
}

int SetDataType(const char *text) {
    if(strcmp(text, "angel") == 0) return INT_t;
    if(strcmp(text, "eve") == 0) return DOUBLE_t;
    if(strcmp(text, "bells") == 0) return CHAR_t;
    if(strcmp(text, "tree") == 0) return STRING_t;
    if(strcmp(text, "carols") == 0) return VOID_t;
    if(strcmp(text, "elf") == 0) return BOOL_t;
    if(strcmp(text, "Christmas") == 0) return CLASS_t;
    return INVALID_t;
}
    ...
    if(AlreadyHaveVariableOrVectorName(fName)) {
        ...
        for(int i = 0; i < nrFunctions; ++i)
            if(strcmp(functions[i].functionName, fName) == 0) {
                if(functions[i].nrParams != 0) good = true; // functiile au acelasi nume, dar nr diferit de parametri
                break;
            }
        if(!good) return;
    ...
    if(AlreadyHaveVariableOrVectorName(fName)) {
        ...
        for(int i = 0; i < nrFunctions; ++i)
            if(strcmp(functions[i].functionName, fName) == 0) {
                ...
                else for (int j = 0; j < nrParams; ++j) // functiile au acelasi nume si nr de parametri, dar de tipuri diferite de date
...
void ExtractVectorName(char *where, char *fromWhere) {
    ...
    do {
        --aux;
    } while(*aux == ' ' || *aux == '\t'); // merg in stanga '[' pana gasesc numele vectorului
    ++aux;
    copy = *aux; // copiez ca sa-l pot restaura
    *aux = 0; // null la sfarsitul numelui vectorului
    strcpy(where, fromWhere);
    *aux = copy; // restaurez fromWhere
}
...
void PrintVec(char *vecName, int pos) {
    int i;
    for(i = 0; i < nrVectors; ++i)
...
void PrintVarOp(struct variable var) {
    switch (var.dataType) {
    case INT_t:
...
void SpecialPrint(char *varName, int type) {
    int i, found = 0;
    for(i = 0; i < nrVars; ++i)
        if(strcmp(varName, vars[i].varName) == 0 && vars[i].dataType == type) {
            Print(varName);
            found = 1;
        }
    if(found == 0) {
        haveError = true;
        strcpy(errorMessage, "Error: variable doesn't have that type! Merry Christmas!");
        return;
    }
}
...
struct variable OperatorFunction(struct variable var1, const char *op, struct variable var2) {
    ...
    if (HaveDifferentTypes(var1, var2)) {
        haveError = true;
        strcpy(errorMessage, "Error: variables have different data types! Merry Christmas!");
        return rez;
    }
    ...
    if (strcmp(op, "&&") == 0) rez = AndFunction(var1, var2);
    if (strcmp(op, "||") == 0) rez = OrFunction(var1, var2);
    if (strcmp(op, "<=") == 0) rez = LeFunction(var1, var2);
    if (strcmp(op, ">=") == 0) rez = GeFunction(var1, var2);
    if (strcmp(op, "==") == 0) rez = EqFunction(var1, var2);
    if (strcmp(op, "!=") == 0) rez = NeqFunction(var1, var2);
    if (strcmp(op, "<") == 0) rez = LsFunction(var1, var2);
    if (strcmp(op, ">") == 0) rez = GrFunction(var1, var2);
...
struct variable SumFunction(struct variable a, struct variable b) {
    ...
    if(a.dataType == INT_t && b.dataType == DOUBLE_t) res = SumFunction(b, a);
...
struct variable TimesFunction(struct variable a, struct variable b) {
    ...
    if(a.dataType == INT_t && b.dataType == DOUBLE_t) res = TimesFunction(b, a);
    return res;
}
...
struct variable AndFunction(struct variable a, struct variable b) {
    ...
    if(a.dataType == INT_t && b.dataType == DOUBLE_t) res = AndFunction(b, a);
...
struct variable OrFunction(struct variable a, struct variable b) {
    struct variable res;
    res.dataType = BOOL_t;
    if (a.dataType == INT_t && b.dataType == INT_t) res.value.boolVal = a.value.intVal || b.value.intVal;
    if (a.dataType == DOUBLE_t && b.dataType == DOUBLE_t) res.value.boolVal = a.value.doubleVal || b.value.doubleVal;
    if (a.dataType == DOUBLE_t && b.dataType == INT_t) res.value.boolVal = a.value.doubleVal || b.value.intVal;
    if (a.dataType == INT_t && b.dataType == DOUBLE_t) res = OrFunction(b, a);
    if (a.dataType == BOOL_t && b.dataType == BOOL_t) res.value.boolVal = a.value.boolVal || b.value.boolVal;
    if (a.dataType == BOOL_t && b.dataType == INT_t) res.value.boolVal = a.value.boolVal || b.value.intVal;
    if (a.dataType == INT_t && b.dataType == BOOL_t) res.value.boolVal = a.value.intVal || b.value.boolVal;
    if (a.dataType == BOOL_t && b.dataType == DOUBLE_t) res.value.boolVal = a.value.boolVal || b.value.doubleVal;
    if (a.dataType == DOUBLE_t && b.dataType == BOOL_t) res.value.boolVal = a.value.doubleVal || b.value.boolVal;
    return res;
}
