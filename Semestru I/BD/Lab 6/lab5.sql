...
select TRIM(s.nume)||' '||TRIM(s.prenume)||' '||TRIM(p.nume)||' '||
TRIM(p.prenume) as "NUME STUDENT NUME PROFI" from studenti s join profesori p on length(s.nume)=length(trim(p.nume));


/*3.Afisati denumirile cursurilor la care s-au pus note cel mult egale cu 8 (<=8).*/
--varianta cu minus
a) select titlu_curs from cursuri c join note n on c.id_curs=n.id_curs MINUS select titlu_curs from cursuri c join note n on
c.id_curs=n.id_curs where valoare >8;	
b)select distinct titlu_curs from cursuri c join note n on c.id_curs=n.id_curs where c.id_curs NOT IN (select id_curs from note
where valoare>8);

/*4.Afisati numele studentilor care au toate notele mai mari ca 7 sau egale cu 7.*/
select s.nume from studenti s join note n on s.nr_matricol=n.nr_matricol MINUS select s.nume from studenti s join note n on
s.nr_matricol=n.nr_matricol where valoare <7;

/*5.Sa se afiseze studentii care au luat nota 7 sau nota 10 la OOP intr-o zi de marti.*/
select nume||' '||prenume as NUME from studenti s join note n on s.nr_matricol=n.nr_matricol join cursuri c on c.id_curs=n.id_curs
where titlu_curs='OOP' and trim(to_char(data_notare,'DAY'))='TUESDAY' and (valoare=7 or valoare=10);
...
select nume,prenume, upper(to_char(data_notare,'month'))||' '||extract (year from data_notare) as sesiune,
 decode(trim(to_char(data_notare,'month')),'february','+') as sesiune_grea from studenti s join note n on s.nr_matricol=
n.nr_matricol where valoare>5;
...
select s.nr_matricol,nume,prenume,nvl(to_char(valoare),'nu are nota') as nota from studenti s left join note 
n on s.nr_matricol=n.nr_matricol;

--3.Afisati DOAR profesorii ce nu sunt titulari de curs.
select TRIM(nume) as prof from profesori p left join didactic d on p.id_prof=d.id_prof where id_curs is NULL;
